...
- Maintain temperature 0 behaviour (no creativity; follow instructions deterministically)
</critical_rules>
...
Construct ONE multi-row INSERT statement covering all new leads. Requirements:
- Columns (exact order): radar_id, first_name, last_name, primary_email, primary_phone, property_address, property_city, property_state, property_zip, assigned_broker_id, property_value, estimated_equity, campaign_archetype
- Provide exactly 13 values per row
- Use `NULL` (unquoted) when email or phone is missing
- Surround text with single quotes, leave numbers unquoted
- Separate rows with commas
- The statement MUST end EXACTLY with  
  `ON CONFLICT (addr_hash) DO NOTHING RETURNING id`
- Ensure parentheses are balanced (every opening `(` has a matching closing `)`)
- Example (for reference only; build with current data):
```sql
INSERT INTO leads (
...
('P104FC74', 'RAYMOND', 'BRANCH', 'email@test.com', '5628418256', '509 W FAIRVIEW BLVD', 'INGLEWOOD', 'CA', '90302', '{{ $json.broker_id }}', 982335, 833337, 'high_equity_special'),
('P104FC77', 'WILEY', 'BRYAN', NULL, '3104622581', '513 W ELLIS AVE', 'INGLEWOOD', 'CA', '90302', '{{ $json.broker_id }}', 902856, 902856, 'cash_unlocked')
ON CONFLICT (addr_hash) DO NOTHING RETURNING id
```

Before calling execute_sql:
1. Verify the statement ends with the required `ON CONFLICT ... RETURNING id`.
2. Count parentheses to ensure balance.
3. Re-read the SQL for stray trailing commas or missing quotes.

Call execute_sql with the validated statement.

If execute_sql returns any syntax/HTTP error:
- Read the error message and identify the issue.
- Rebuild the SQL to correct it.
- Retry execute_sql ONCE with the corrected statement.
- If the second attempt fails, stop further steps for this broker, log a structured failure summary (include the error text and offending SQL snippet), and exit the workflow.
