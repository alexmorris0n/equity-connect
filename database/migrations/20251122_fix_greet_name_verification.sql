-- Fix GREET prompt to verify caller name before using it
-- Issue: LiveKit greets with stored name without verification, causing wrong person scenarios
-- Solution: Ask for name first if stored name exists, verify it matches, use mark_wrong_person if different
...
-- Also add mark_wrong_person to greet tools if not already there
UPDATE prompt_versions
SET content = jsonb_set(
  content,
  '{tools}',
  COALESCE(
    CASE 
      ...
      WHEN content->>'tools'::text LIKE '%mark_wrong_person%' THEN content->'tools'
      ELSE (content->'tools' || '["mark_wrong_person"]'::jsonb)::jsonb
    END,
    '["mark_wrong_person", "mark_greeted", "route_conversation"]'::jsonb
  )
)
WHERE prompt_id IN (
  SELECT id FROM prompts 
...
UPDATE prompt_versions
SET is_active = true,
    change_summary = 'Added name verification to prevent wrong person scenarios - now asks for name confirmation if stored name exists'
WHERE prompt_id IN (
  SELECT id FROM prompts 
  WHERE node_name = 'greet' 
    AND vertical = 'reverse_mortgage' 
