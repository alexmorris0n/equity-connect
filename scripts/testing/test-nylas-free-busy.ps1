# Test Nylas Free/Busy endpoint (the correct one from tools.js)
# This is what Barbara actually uses for availability checking
...
Write-Host ""
Write-Host "Testing Nylas Free/Busy endpoint (from tools.js)..." -ForegroundColor Cyan
...
# Test Free/Busy endpoint (the correct one from tools.js)
Write-Host "Test: Free/Busy endpoint..." -ForegroundColor Yellow
try {
    $url = "https://api.us.nylas.com/v3/calendars/free-busy"
    
    $startTime = [DateTimeOffset]::new((Get-Date)).ToUnixTimeSeconds()
    $endTime = [DateTimeOffset]::new((Get-Date).AddDays(14)).ToUnixTimeSeconds()
...
Write-Host ""
Write-Host "Free/Busy test complete!" -ForegroundColor Cyan
