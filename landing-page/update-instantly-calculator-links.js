...
// Node.js 18+ has fetch built-in, no need to require it
...
async function updateInstantlyLeads() {
  ...
  for (const lead of leads) {
    ...
    try {
      ...
      // Step 2: Update the lead with just the TOKEN (not full URL)
      const patchResponse = await fetch(`${INSTANTLY_API_BASE}/leads/${instantlyLeadId}`, {
        ...
        body: JSON.stringify({
          payload: {
            calculatorToken: token,  // Just the token!
      ...
      console.log(`✅ Updated ${lead.primary_email} - token: ${token}`)
      updated++
